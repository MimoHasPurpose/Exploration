...
def Inverse_kinematics(target):
    ...
    angle_1 = math.atan(z / y) - (math.atan((l2 * math.sin(-angle_2)) / (l1 + l2 * math.cos(-angle_2))))
...
while True:
    ...
    y_new = r * math.cos(theta) + 1       # Cartesian to Polar Cordinate Transformation
    z_new = r * math.sin(theta) + 0.5

    if use_custom:
        angle_1, angle_2 = Inverse_kinematics([y_new, z_new])
    else:
    ...
    p.addUserDebugLine([0, y_old, z_old], [0, y_new, z_new], [1,0,0])   # For Visualization
